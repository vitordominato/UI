...
for i, leito in enumerate(leitos):
    ...
    if st.session_state.get("editando") == chave:
        with st.expander(f"Editar Leito {leito} - {unidade} - {andar}", expanded=True):
            nome = st.text_input("Nome do paciente", value=nome, key=f"nome_{chave}")
            medico = st.selectbox("Médico responsável", options=lista_medicos, key=f"medico_{chave}") if lista_medicos else st.text_input("Médico responsável", key=f"medico_text_{chave}")
