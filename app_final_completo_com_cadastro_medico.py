...
if pagina == "Painel de Internações":
    ...
    with st.form("form_medico"):
        ...
        if salvar and novo_medico:
            df_medicos = pd.read_excel("Cadastro_Medicos.xlsx")
            novo = pd.DataFrame([{"Nome do Médico": novo_medico}])
            df_medicos = pd.concat([df_medicos, novo], ignore_index=True)
            df_medicos.to_excel("Cadastro_Medicos.xlsx", index=False)
