...
if pagina == "Painel de Internações":
    ...
    if df_pacientes.empty:
        ...
        with st.sidebar.expander("Filtros"):
            def safe_select(label, coluna, opcoes=None):
                if coluna in df_pacientes.columns and not df_pacientes[coluna].dropna().empty:
                    opcoes_dinamicas = sorted(df_pacientes[coluna].dropna().astype(str).unique().tolist())
                    return st.selectbox(label, ["Todos"] + opcoes_dinamicas)
                elif opcoes:
            ...
            unidade = safe_select("Unidade", "Unidade")
