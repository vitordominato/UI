...
with st.sidebar:
    ...
    unidades = sorted([u for u in df_leitos_sidebar['unidade'].dropna().unique().tolist() if u])
    unidade_sel = st.selectbox("Unidade", unidades, index=0 if unidades else None, key="unidade_filtro") if unidades else None
    df_filtrado_sidebar = df_leitos_sidebar[df_leitos_sidebar['unidade'] == unidade_sel] if unidade_sel else df_leitos_sidebar
    andares = sorted([a for a in df_filtrado_sidebar['andar'].dropna().unique().tolist() if a])
    andar_sel = st.selectbox("Andar", andares, index=0 if andares else None, key="andar_filtro") if andares else None
    df_filtrado_sidebar = df_filtrado_sidebar[df_filtrado_sidebar['andar'] == andar_sel] if andar_sel else df_filtrado_sidebar
    medicos = sorted([m for m in df_filtrado_sidebar['medico'].dropna().unique().tolist() if m])
...
if aba_selecionada == "Leitos":
    ...
    for _, leito in df_filtrado.iterrows():
        ...
        ficha_clinica_form(leito, leito['leito'], df_leitos, key_prefix=f"ficha_{leito['leito']}")
        st.markdown("---")
