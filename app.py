...
if aba_selecionada == "Leitos":
    if not GOOGLE_SHEETS_OK:
        ...
        if andar in CONFIG_ANDARES[unidade]:
            ...
            inicio, fim = 0, 0
        df_leitos = df_leitos_gs.copy()
        leitos_andar = df_leitos[(df_leitos['leito'].astype(str).str.isdigit()) & (df_leitos['leito'].astype(int) >= inicio) & (df_leitos['leito'].astype(int) <= fim)]
        st.dataframe(leitos_andar, use_container_width=True, hide_index=True)
