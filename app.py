...
from google_sheets_utils import salvar_df_no_google_sheets, importar_df_do_google_sheets
from datetime import datetime

GOOGLE_CREDS_PATH = 'secrets/irah-app-14b9444a55a6.json'

# Importação automática dos dados do Google Sheets para CSV local (Leitos e Médicos)
def importar_se_vazio(local_path, nome_planilha, nome_aba):
    import pandas as pd
    import os
    if not os.path.exists(local_path) or pd.read_csv(local_path, dtype=str).empty:
        df_remoto = importar_df_do_google_sheets(nome_planilha, nome_aba, cred_path=GOOGLE_CREDS_PATH)
        if not df_remoto.empty:
            df_remoto.to_csv(local_path, index=False)
            import streamlit as st
            st.info(f"Dados de '{nome_aba}' importados do Google Sheets para o aplicativo.")

importar_se_vazio('data/leitos.csv', 'hospital_app_dados', 'leitos')
importar_se_vazio('data/medicos.csv', 'hospital_app_dados', 'medicos')
...
# Variáveis globais para status de sincronização
if 'last_sync' not in st.session_state:
    st.session_state['last_sync'] = None
if 'sync_status' not in st.session_state:
    st.session_state['sync_status'] = 'Nunca sincronizado'
...
def sincronizar_tudo():
    try:
        st.session_state['sync_status'] = 'Sincronizando...'
        # --- LEITOS ---
        local_path_leitos = 'data/leitos.csv'
        df_local_leitos = pd.read_csv(local_path_leitos, dtype=str).fillna("") if os.path.exists(local_path_leitos) else pd.DataFrame()
        df_sheet_leitos = importar_df_do_google_sheets('hospital_app_dados', 'leitos', cred_path=GOOGLE_CREDS_PATH)
        if not df_sheet_leitos.empty:
            # Merge inteligente por coluna 'leito' (chave única)
            if not df_local_leitos.empty:
                # Atualiza registros existentes e adiciona novos
                df_merged = pd.merge(df_local_leitos, df_sheet_leitos, on='leito', how='outer', suffixes=('_local', '_sheet'), indicator=True)
                # Para cada coluna, prioriza o dado local se existir, senão o da planilha
                def pick_value(row, col):
                    local = row.get(f"{col}_local", "")
                    sheet = row.get(f"{col}_sheet", "")
                    return local if local != "" else sheet
                final_cols = [col for col in df_sheet_leitos.columns]  # Garante todas as colunas da planilha
                merged_data = {col: [pick_value(row, col) for _, row in df_merged.iterrows()] for col in final_cols}
                df_final = pd.DataFrame(merged_data)
            else:
                df_final = df_sheet_leitos
            df_final.to_csv(local_path_leitos, index=False)
        # --- MÉDICOS ---
        local_path_medicos = 'data/medicos.csv'
        df_local_medicos = pd.read_csv(local_path_medicos, dtype=str).fillna("") if os.path.exists(local_path_medicos) else pd.DataFrame()
        df_sheet_medicos = importar_df_do_google_sheets('hospital_app_dados', 'medicos', cred_path=GOOGLE_CREDS_PATH)
        if not df_sheet_medicos.empty:
            # Merge inteligente por coluna 'nome' (chave única)
            if not df_local_medicos.empty:
                df_merged = pd.merge(df_local_medicos, df_sheet_medicos, on='nome', how='outer', suffixes=('_local', '_sheet'), indicator=True)
                def pick_value_med(row, col):
                    local = row.get(f"{col}_local", "")
                    sheet = row.get(f"{col}_sheet", "")
                    return local if local != "" else sheet
                final_cols_med = [col for col in df_sheet_medicos.columns]
                merged_data_med = {col: [pick_value_med(row, col) for _, row in df_merged.iterrows()] for col in final_cols_med}
                df_final_med = pd.DataFrame(merged_data_med)
            else:
                df_final_med = df_sheet_medicos
            df_final_med.to_csv(local_path_medicos, index=False)
        st.session_state['last_sync'] = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
        st.session_state['sync_status'] = f'Sincronizado em {st.session_state["last_sync"]}'
        st.session_state['sync_error'] = ''
    except Exception as e:
        st.session_state['sync_status'] = 'Erro na sincronização'
        st.session_state['sync_error'] = str(e)

# --- AVISO SOBRE AMBIENTE EFÊMERO ---
import platform
if any(env in platform.platform().lower() for env in ["heroku", "streamlit"]):
    st.warning("AVISO: Este ambiente pode apagar dados locais ao hibernar/desligar. Certifique-se de sincronizar alterações importantes com o Google Sheets para não perder informações.")

# Thread de sincronização periódica

def sync_periodico():
    while True:
        sincronizar_tudo()  # Agora só importa do Sheets para CSV
        time.sleep(50*60)  # 50 minutos

if 'sync_thread_started' not in st.session_state:
    thread = threading.Thread(target=sync_periodico, daemon=True)
    thread.start()
    st.session_state['sync_thread_started'] = True

# Botão de sincronizar agora no menu lateral
with st.sidebar:
    if st.button('🔄 Sincronizar Agora'):
        sincronizar_tudo()
    # Status
    st.markdown(f"**Status da sincronização:** {st.session_state['sync_status']}")
    if st.session_state['sync_error']:
        st.error(f"Erro: {st.session_state['sync_error']}")
...
def ficha_clinica_form(info_leito, leito, df_leitos, caminho_leitos, key_prefix="ficha"):
    ...
    if alterou_ficha:
        ...
        salvar_df_no_google_sheets(df_leitos, 'hospital_app_dados', 'leitos')
    ...
    if os.path.exists(CAMINHO_LEITOS):
        ...
        with col_irah_btns[0]:
            if st.button("Atualizar risco assistencial na ficha clínica", key="atualizar_irah"):
                ...
                historico_path = os.path.join('data', 'historico_irah.csv')
...
if aba_selecionada == "Leitos":
    ...
    # Botões de importar/exportar removidos. Sincronização automática implementada abaixo.
    ...
    if len(df_leitos) == 0:
        ...
        for leito in leitos_range:
            ...
            with st.container():
                ...
                # Salvamento automático + sincronização Google Sheets
                alterou = (
    ...
    if os.path.exists(CAMINHO_LEITOS):
        ...
        if not pendencias_tarde.empty:
            for idx, row in pendencias_tarde.iterrows():
                ...
                cols[4].markdown(f"<b>Tipo:</b> {row['pendencia_rotina']}", unsafe_allow_html=True)
                cols[5].markdown(f"<b>Descrição:</b> {row['descricao_pendencia']}", unsafe_allow_html=True)
                if cols[6].button("Realizado", key=f"realizado_tarde_{row['leito']}_{idx}"):
                    df_leitos_pendencia.loc[df_leitos_pendencia['leito'] == row['leito'], 'pendencia_resolvida'] = 'Sim'
                    df_leitos_pendencia.to_csv(CAMINHO_LEITOS, index=False)
                    st.success(f"Pendência do leito {row['leito']} marcada como realizada.")
                    st.experimental_rerun()
    ...
    with st.expander("Cadastrar novo médico", expanded=True):
        with st.form(key="form_cadastro_medico"):
            ...
            if submit:
                ...
                elif nome_medico.strip() not in df_medicos['Nome do Médico'].values:
