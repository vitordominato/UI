...
if aba_selecionada == "Leitos":
    if not GOOGLE_SHEETS_OK:
        ...
        for idx, row in leitos_andar.iterrows():
            ...
            # Exibir cabeçalho com dados principais do leito
            st.markdown(f"#### Leito {row['leito']}")
            col1, col2, col3 = st.columns(3)
            col1.write(f"**Paciente:** {row.get('nome', '')}")
            col2.write(f"**Médico:** {row.get('medico', '')}")
            col3.write(f"**Equipe:** {row.get('equipe', '')}")
