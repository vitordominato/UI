...
def sincronizar_tudo():
    try:
        ...
        # Leitos
        if os.path.exists('data/leitos.csv'):
            df_leitos = pd.read_csv('data/leitos.csv', dtype=str).fillna("")
            salvar_df_no_google_sheets(df_leitos, 'hospital_app_dados', 'leitos', cred_path=GOOGLE_CREDS_PATH)
        # Médicos
        if os.path.exists('data/medicos.csv'):
            df_medicos = pd.read_csv('data/medicos.csv', dtype=str).fillna("")
            salvar_df_no_google_sheets(df_medicos, 'hospital_app_dados', 'medicos', cred_path=GOOGLE_CREDS_PATH)
