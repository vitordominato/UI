...
with st.sidebar:
    ...
    # Filtro de unidade: todas as unidades possíveis
    unidades = sorted([u for u in df_leitos_sidebar['unidade'].dropna().unique() if u])
    unidade_sel = st.selectbox("Unidade", ["(Todas)"] + unidades, index=0, key="sidebar_unidade_filtro") if unidades else "(Todas)"
    # Filtro de andar: todos os andares possíveis da unidade selecionada (ou todos)
    if unidade_sel and unidade_sel != "(Todas)":
        andares = sorted([a for a in df_leitos_sidebar[df_leitos_sidebar['unidade'] == unidade_sel]['andar'].dropna().unique() if a])
    else:
        andares = sorted([a for a in df_leitos_sidebar['andar'].dropna().unique() if a])
    andar_sel = st.selectbox("Andar", ["(Todos)"] + andares, index=0, key="sidebar_andar_filtro") if andares else "(Todos)"
