...
try:
    ...
    df_leitos_gs.to_csv(os.path.join('data', 'leitos.csv'), index=False)
    # Admissões Pendentes
    df_admissoes_pendentes_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_ADMISSOES_PENDENTES)
    df_admissoes_pendentes_gs.to_csv(os.path.join('data', 'admissoes_pendentes.csv'), index=False)
    # Admitidos
    df_admitidos_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_ADMITIDOS)
    df_admitidos_gs.to_csv(os.path.join('data', 'admitidos.csv'), index=False)
    # IRAH
    df_irah_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_IRAH)
    df_irah_gs.to_csv(os.path.join('data', 'irah.csv'), index=False)
    # Médicos
    df_medicos_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_MEDICOS)
    df_medicos_gs.to_csv(os.path.join('data', 'medicos.csv'), index=False)
    st.info('Dados carregados do Google Sheets e sincronizados localmente.')
except Exception as e:
...
def salvar_admissoes(df):
    ...
    try:
        ...
        st.warning(f'Falha ao sincronizar admissões pendentes com Google Sheets: {e}')
def salvar_admitidos(df):
    ...
    try:
        ...
        st.warning(f'Falha ao sincronizar admitidos com Google Sheets: {e}')
...
if aba_selecionada == "Leitos":
    ...
    if col_leitos2.button('Importar', help='Importar dados dos leitos do Google Sheets', key='importar_leitos'):
        from google_sheets_utils import importar_df_do_google_sheets
        df_importado = importar_df_do_google_sheets('hospital_app_dados', 'leitos', None)
        if not df_importado.empty:
            df_importado.to_csv(os.path.join('data', 'leitos.csv'), index=False)
            st.success('Dados de leitos importados do Google Sheets e atualizados com sucesso!')
        else:
            st.warning('Nenhum dado foi importado do Google Sheets. Verifique se a planilha/aba está preenchida.')
    if col_leitos3.button('Exportar', help='Exportar dados dos leitos para Google Sheets', key='exportar_leitos'):
        from google_sheets_utils import salvar_df_no_google_sheets
        df_leitos = pd.read_csv(os.path.join('data', 'leitos.csv'))
        ...
        st.success('Dados de leitos exportados para Google Sheets com sucesso!')
