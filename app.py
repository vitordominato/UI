...
# Criação de tabs
abas = ["Leitos", "Visão do Plantonista", "Pendência tarde", "Admissão", "Médicos", "Gestão", "Dashboard", "🧮 IRAH"]
aba_selecionada = st.sidebar.radio("", abas)
    ...
    if len(df_leitos) == 0:
        ...
        for leito in leitos_range:
            ...
            with st.container():
                ...
                opcoes_equipe = [""] + ["Clínica Médica", "Cardiologia", "Hepatologia", "Cirurgia Geral", "Ortopedia", "Obstetrícia", "Pediatria", "Médico Assistente", "Hematologia", "Oncologia"]
        ...
        with st.expander("Desospitalização", expanded=True):
            ...
            if not desosp.empty:
                for idx, row in desosp.iterrows():
                    ...
                    # Registrar histórico de desospitalização (apenas visualização, não há ação editável aqui)
                    # Se desejar registrar apenas quando houver alteração, mova para o local de edição.
                    st.markdown('<hr style="margin-top: 0.5rem; margin-bottom: 0.5rem;">', unsafe_allow_html=True)
            else:
