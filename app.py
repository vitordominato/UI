...
def limpar_leito(leito_id):
    dados_vazios = {col: "" for col in essential_columns}
...
def painel_edicao_leito(leito):
    ...
    # Campos cadastrais principais na primeira linha
    campos_cadastrais = ['leito', 'nome', 'medico', 'equipe']
    ...
    st.markdown("---")
    # Ficha clínica independente
    # Garante que df_leitos está disponível no escopo global
    global df_leitos
    info_leito = df_leitos[df_leitos['leito'] == leito_id] if not df_leitos.empty else None
    if info_leito is not None and not info_leito.empty:
        ficha_clinica_form(info_leito, leito_id, df_leitos, caminho_leitos=None, key_prefix=f"ficha_{leito_id}")
    else:
...
from firebase_utils import salvar_ficha_clinica, obter_ficha_clinica
