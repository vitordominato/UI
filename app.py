...
def sincronizar_tudo():
    try:
        ...
        # Leitos
        df_leitos = importar_df_do_google_sheets('hospital_app_dados', 'leitos', cred_path=GOOGLE_CREDS_PATH)
        if not df_leitos.empty:
            df_leitos.to_csv('data/leitos.csv', index=False)
        # Médicos
        df_medicos = importar_df_do_google_sheets('hospital_app_dados', 'medicos', cred_path=GOOGLE_CREDS_PATH)
