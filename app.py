...
if "editando" in st.session_state and st.session_state.get("modo") == "cadastro":
    ...
    # Verificação de unidade e andar válidos
    ...
    unidade = st.selectbox("Unidade", list(estrutura_leitos.keys()),
                           index=list(estrutura_leitos.keys()).index(st.session_state.unidade_selecionada))
    andar = st.selectbox("Andar", list(estrutura_leitos[unidade].keys()),
                         index=list(estrutura_leitos[unidade].keys()).index(st.session_state.andar_selecionado))
...
if "unidade_selecionada" not in st.session_state or st.session_state.unidade_selecionada not in estrutura_leitos:
    st.session_state.unidade_selecionada = list(estrutura_leitos.keys())[0]
if "andar_selecionado" not in st.session_state or \
   st.session_state.andar_selecionado not in estrutura_leitos[st.session_state.unidade_selecionada]:
    st.session_state.andar_selecionado = list(estrutura_leitos[st.session_state.unidade_selecionada].keys())[0]
    st.session_state.andar_selecionado = list(estrutura_leitos[st.session_state.unidade_selecionada].keys())[0]
        st.session_state.andar_selecionado = list(estrutura_leitos[st.session_state.unidade_selecionada].keys())[0]
...
leitos_existentes = estrutura_leitos[unidade][andar]
df_unidade_andar = df_leitos[(df_leitos["unidade"] == unidade) & (df_leitos["andar"] == andar)]
leitos = sorted(leitos_existentes, key=lambda x: int(x))
