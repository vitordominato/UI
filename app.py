...
import backup_data  # Importa o módulo de backup
from google_sheets_utils import importar_df_do_google_sheets, salvar_df_no_google_sheets


st.set_page_config(page_title="Gestão de Leitos", layout="wide")

# Executa backup automático dos dados ao iniciar o app
try:
    backup_data.backup_csv_files()
    st.info('Backup automático dos dados realizado com sucesso.')
except Exception as e:
    st.warning(f'Falha ao realizar backup automático: {e}')

# Carrega dados do Google Sheets e sobrescreve arquivos locais (fonte principal)
from google_sheets_utils import importar_df_do_google_sheets
NOME_PLANILHA = 'hospital_app_dados'  # Nome da sua planilha
ABA_LEITOS = 'leitos'
ABA_ADMISSOES_PENDENTES = 'admissoes pendentes'
ABA_ADMITIDOS = 'admitidos'
ABA_IRAH = 'IRAH'
ABA_MEDICOS = 'medicos'
try:
    # Leitos
    df_leitos_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_LEITOS)
    # Admissões Pendentes
    df_admissoes_pendentes_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_ADMISSOES_PENDENTES)
    # Admitidos
    df_admitidos_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_ADMITIDOS)
    # IRAH
    df_irah_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_IRAH)
    # Médicos
    df_medicos_gs = importar_df_do_google_sheets(NOME_PLANILHA, ABA_MEDICOS)
    st.info('Dados carregados do Google Sheets.')
    GOOGLE_SHEETS_OK = True
except Exception as e:
    st.error(f'Falha ao carregar dados do Google Sheets: {e}')
    GOOGLE_SHEETS_OK = False

# Dados principais do Google Sheets
NOME_PLANILHA = 'hospital_app_dados'
ABA_LEITOS = 'leitos'
ABA_ADMISSOES_PENDENTES = 'admissoes pendentes'
ABA_ADMITIDOS = 'admitidos'
ABA_IRAH = 'IRAH'
...
def ficha_clinica_form(info_leito, leito, df_leitos, key_prefix="ficha"):
    ...
    if alterou_ficha:
        ...
        df_leitos.to_csv('data/leitos.csv', index=False)
        # Sincronização automática com Google Sheets
        try:
            salvar_df_no_google_sheets(df_leitos, NOME_PLANILHA, ABA_LEITOS)
        except Exception as e:
            st.warning(f'Falha ao sincronizar leitos com Google Sheets: {e}')
    ...
    if col1.button("🟩 Alta", key=f"{key_prefix}_alta_{leito}"):
        ...
        ALTAS_HOJE_PATH = 'data/altas_hoje.csv'
        ...
        st.success("Paciente adicionado à lista de altas do dia!")
...
ADMISSOES_PATH = 'data/admissoes.csv'
ADMITIDOS_PATH = 'data/admitidos.csv'
HISTORICO_ADMISSOES_PATH = 'data/historico_admissoes.csv'
HISTORICO_AUTORIZACAO_PATH = 'data/historico_autorizacao.csv'
HISTORICO_DESOSP_PATH = 'data/historico_desospitalizacao.csv'
...
def registrar_historico_admissao(acao, nome_paciente, origem, observacoes, detalhes_adicionais=None):
    ...
    import csv
    file_exists = os.path.exists(HISTORICO_ADMISSOES_PATH)
    with open(HISTORICO_ADMISSOES_PATH, 'a', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['data_hora','acao','nome_paciente','origem','observacoes','detalhes_adicionais'])
        if not file_exists:
            writer.writeheader()
...
if not os.path.exists(HISTORICO_AUTORIZACAO_PATH):
    pd.DataFrame(columns=["data_hora","acao","leito","paciente","detalhe","observacoes"]).to_csv(HISTORICO_AUTORIZACAO_PATH, index=False)
if not os.path.exists(HISTORICO_DESOSP_PATH):
    pd.DataFrame(columns=["data_hora","acao","leito","paciente","detalhe","observacoes"]).to_csv(HISTORICO_DESOSP_PATH, index=False)

def registrar_historico_autorizacao(acao, leito, paciente, detalhe, observacoes=None):
    data_hora = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
    linha = {
        'data_hora': data_hora,
        'acao': acao,
    ...
    import csv
    file_exists = os.path.exists(HISTORICO_AUTORIZACAO_PATH)
    with open(HISTORICO_AUTORIZACAO_PATH, 'a', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['data_hora','acao','leito','paciente','detalhe','observacoes'])
        if not file_exists:
            writer.writeheader()
        writer.writerow(linha)

def registrar_historico_desospitalizacao(acao, leito, paciente, detalhe, observacoes=None):
    data_hora = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
    linha = {
        'data_hora': data_hora,
        'acao': acao,
    ...
    import csv
    file_exists = os.path.exists(HISTORICO_DESOSP_PATH)
    with open(HISTORICO_DESOSP_PATH, 'a', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['data_hora','acao','leito','paciente','detalhe','observacoes'])
        if not file_exists:
            writer.writeheader()
        writer.writerow(linha)
...
def salvar_admissoes(df):
    if not GOOGLE_SHEETS_OK:
        st.error('Não é possível salvar admissões: conexão com Google Sheets indisponível.')
        return
    try:
        salvar_df_no_google_sheets(df, NOME_PLANILHA, ABA_ADMISSOES_PENDENTES)
        st.success('Admissões pendentes salvas no Google Sheets.')
    except Exception as e:
        st.error(f'Falha ao sincronizar admissões pendentes com Google Sheets: {e}')
def salvar_admitidos(df):
    if not GOOGLE_SHEETS_OK:
        st.error('Não é possível salvar admitidos: conexão com Google Sheets indisponível.')
        return
    try:
        salvar_df_no_google_sheets(df, NOME_PLANILHA, ABA_ADMITIDOS)
        st.success('Admitidos salvos no Google Sheets.')
    except Exception as e:
        st.error(f'Falha ao sincronizar admitidos com Google Sheets: {e}')
...
# Carregar dados dos médicos diretamente do Google Sheets
if GOOGLE_SHEETS_OK:
    df_medicos = df_medicos_gs.copy()
    ...
    if os.path.exists(CAMINHO_LEITOS):
        ...
        with col_irah_btns[0]:
            if st.button("Atualizar risco assistencial na ficha clínica", key="atualizar_irah"):
                ...
                historico_path = 'data/historico_irah.csv'
                ...
                if os.path.exists(historico_path):
                    historico.to_csv(historico_path, mode='a', header=False, index=False)
                else:
                    historico.to_csv(historico_path, mode='w', header=True, index=False)
                st.success("Risco assistencial atualizado e histórico salvo!")
        with col_irah_btns[1]:
            # Exportar histórico IRAH para Google Sheets
            if st.button("Exportar histórico IRAH", key="exportar_irah_gs"):
                import pandas as pd
                from google_sheets_utils import salvar_df_no_google_sheets
                historico_path = 'data/historico_irah.csv'
                if os.path.exists(historico_path):
                    df_hist = pd.read_csv(historico_path, dtype=str).fillna("")
                    ...
                    if not df_hist.empty:
                        ...
                        if 'data' not in df_hist.columns:
                            df_hist['data'] = pd.to_datetime(df_hist['data_hora'], format='%d/%m/%Y %H:%M:%S', errors='coerce').dt.strftime('%Y-%m-%d')
                        salvar_df_no_google_sheets(df_hist, 'hospital_app_dados', 'IRAH')
                        st.success('Histórico IRAH exportado para Google Sheets com sucesso!')
                        # Sincronização automática IRAH após qualquer alteração
                        try:
                            salvar_df_no_google_sheets(df_hist, NOME_PLANILHA, ABA_IRAH)
                        except Exception as e:
                            st.warning(f'Falha ao sincronizar IRAH com Google Sheets: {e}')
                    else:
                        st.info('Nenhum histórico IRAH para exportar.')
...
if aba_selecionada == "Leitos":
    if not GOOGLE_SHEETS_OK:
        st.error('Não é possível exibir ou editar leitos: conexão com Google Sheets indisponível.')
    else:
        ...
        if andar in CONFIG_ANDARES[unidade]:
            ...
            inicio, fim = 0, 0
        df_leitos = df_leitos_gs.copy()
        # Filtra apenas leitos numéricos para evitar erro de conversão
        mask_numericos = df_leitos['leito'].astype(str).str.isdigit()
        df_leitos_num = df_leitos[mask_numericos].copy()
        df_leitos_num['leito_int'] = df_leitos_num['leito'].astype(int)
        leitos_andar = df_leitos_num[(df_leitos_num['leito_int'] >= inicio) & (df_leitos_num['leito_int'] <= fim)]
        # NÃO exibir a planilha completa! Exibir apenas interface de interação para cada leito.
        for idx, row in leitos_andar.iterrows():
            info_leito = df_leitos[df_leitos['leito'] == row['leito']]
            header = f"Leito {row['leito']} | Paciente: {row.get('nome', '')} | Médico: {row.get('medico', '')} | Equipe: {row.get('equipe', '')}"
            with st.expander(header, expanded=False):
                ficha_clinica_form(info_leito, row['leito'], df_leitos, key_prefix="painel")
...
if aba_selecionada == "Admissão":
    ...
    for idx, row in df_admitidos.iterrows():
        ...
        if c5.button("Desfazer", key=desfazer_key, type="secondary"):
            ...
            registrar_historico_admissao("admissao_desfeita", row['nome_paciente'], row['origem'], row['observacoes'])
...
if aba_selecionada == "Médicos":
    ...
    if col_med2.button('Importar', help='Importar dados dos médicos do Google Sheets', key='importar_medicos'):
        from google_sheets_utils import importar_df_do_google_sheets
        df_importado = importar_df_do_google_sheets('hospital_app_dados', 'medicos', None)
        if not df_importado.empty:
            df_importado.to_csv(os.path.join('data', 'medicos.csv'), index=False)
            st.success('Dados de médicos importados do Google Sheets e atualizados com sucesso!')
        else:
            st.warning('Nenhum dado foi importado do Google Sheets. Verifique se a planilha/aba está preenchida.')
    if col_med3.button('Exportar', help='Exportar dados dos médicos para Google Sheets', key='exportar_medicos'):
        from google_sheets_utils import salvar_df_no_google_sheets
        df_medicos = pd.read_csv(os.path.join('data', 'medicos.csv'))
        salvar_df_no_google_sheets(
            df_medicos,
            'hospital_app_dados',
            'medicos',
            None
        )
        st.success('Dados de médicos exportados para Google Sheets com sucesso!')
            ...
            if submit:
                if nome_medico.strip() == "":
                    ...
                    if nome_medico.strip() not in df_medicos['Nome do Médico'].values:
                        novo_medico = {"Nome do Médico": nome_medico.strip()}
                        df_medicos = pd.concat([df_medicos, pd.DataFrame([novo_medico])], ignore_index=True)
                        df_medicos.to_csv(CAMINHO_MEDICOS, index=False)
                        # Sincronização automática com Google Sheets após cadastro de médico
                        try:
                            salvar_df_no_google_sheets(df_medicos, NOME_PLANILHA, ABA_MEDICOS)
                        except Exception as e:
                            st.warning(f'Falha ao sincronizar médicos com Google Sheets: {e}')
    ...
    if not df_medicos.empty:
        for idx, row in df_medicos.iterrows():
            ...
            if col_excluir.button("Excluir", key=f"excluir_medico_{idx}"):
                df_medicos_novo = df_medicos.drop(idx).reset_index(drop=True)
                df_medicos_novo.to_csv(CAMINHO_MEDICOS, index=False)
                # Sincronização automática com Google Sheets após exclusão de médico
                try:
                    salvar_df_no_google_sheets(df_medicos_novo, NOME_PLANILHA, ABA_MEDICOS)
                except Exception as e:
                    st.warning(f'Falha ao sincronizar médicos com Google Sheets: {e}')
                st.experimental_rerun()
    else:
...
if aba_selecionada == "Gestão":
    ...
    if os.path.exists(CAMINHO_LEITOS):
        df_leitos_gestao = pd.read_csv(CAMINHO_LEITOS, dtype=str).fillna("")
        # Considera tanto 'Alto' quanto 'alto' para robustez
        altos = df_leitos_gestao[df_leitos_gestao['risco_assistencial'].str.lower() == 'alto']
        if altos.empty:
            ...
            if not autorizacao.empty:
                for idx, row in autorizacao.iterrows():
                    ...
                    if (
                        ...
                        df_gestao.to_csv(CAMINHO_LEITOS, index=False)
                        st.experimental_rerun()
                    st.markdown('<hr style="margin-top: 0.5rem; margin-bottom: 0.5rem;">', unsafe_allow_html=True)
            else:
