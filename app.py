...
def painel_edicao_leito(leito):
    ...
    obrigatorios = ['unidade', 'andar']
    campos_vazios = []
    for idx, col_name in enumerate(campos_cadastrais):
        valor = st.session_state.get(f"{col_name}_{leito_id}", leito.get(col_name, ""))
        cols[idx].text_input(
            col_name.replace('_', ' ').capitalize() + (" *" if col_name in obrigatorios else ""),
            value=valor,
            key=f"{col_name}_{leito_id}"
        )
        if col_name in obrigatorios and (valor is None or str(valor).strip() == ""):
            campos_vazios.append(col_name)

    if campos_vazios:
        st.warning(f"Preencha os campos obrigatórios: {', '.join([c.capitalize() for c in campos_vazios])}")
    else:
        # Só ativa salvamento automático se todos obrigatórios estiverem preenchidos
        for col_name in obrigatorios:
            st.session_state[f"{col_name}_preenchido_{leito_id}"] = True
        st.button("Salvar alterações", key=f"btn_salvar_{leito_id}", on_click=salvar_leito_automatico, args=(leito_id,))
    # Se quiser impedir alteração de outros campos até obrigatórios serem preenchidos, pode desabilitar os inputs acima conforme necessidade.
    st.markdown("---")
...
with st.sidebar:
    ...
    df_leitos_sidebar = garantir_colunas(get_leitos_cached())
    # Padrão de unidades e andares conforme enviado pelo usuário
    PADRAO_UNIDADES_ANDARES = {
        "Unidade I": [
            "4º andar", "5º andar", "6º andar"
        ],
        "Unidade III": [
            "4º andar", "5º andar", "6º andar", "7º andar", "8º andar", "9º andar"
        ],
        "Unidade IV": [
            "6º andar (TMO)", "7º andar (Cardiologia)", "8º andar", "9º andar"
        ]
    }
    unidades = list(PADRAO_UNIDADES_ANDARES.keys())
    unidade_sel = st.selectbox("Unidade", ["(Todas)"] + unidades, index=0, key="sidebar_unidade_filtro")
    # Filtro de andar: apenas os andares válidos da unidade selecionada (ou todos do padrão)
    if unidade_sel and unidade_sel != "(Todas)":
        andares = PADRAO_UNIDADES_ANDARES[unidade_sel]
    else:
        # Todos os andares únicos do padrão, sem duplicidade
        andares = []
        for lista in PADRAO_UNIDADES_ANDARES.values():
            andares.extend(lista)
        andares = sorted(list(dict.fromkeys(andares)), key=lambda x: (x.split('º')[0], x))
    andar_sel = st.selectbox("Andar", ["(Todos)"] + andares, index=0, key="sidebar_andar_filtro")
    # Filtro de médico: todos os médicos possíveis do filtro atual
    if unidade_sel and unidade_sel != "(Todas)":
        df_temp = df_leitos_sidebar[df_leitos_sidebar['unidade'] == unidade_sel]
    else:
        df_temp = df_leitos_sidebar
    if andar_sel and andar_sel != "(Todos)":
        df_temp = df_temp[df_temp['andar'] == andar_sel]
    medicos = sorted([m for m in df_temp['medico'].dropna().unique() if m])
    medico_sel = st.selectbox("Filtrar por médico", ["(Todos)"] + medicos, index=0, key="sidebar_medico_filtro") if medicos else "(Todos)"

# --- Conteúdo principal ---
if aba_selecionada == "Leitos":
    st.header("Gestão de Leitos")
    df_leitos = garantir_colunas(get_leitos_cached())
    # Reaplique os filtros do menu lateral
    df_filtrado = df_leitos.copy()
    if unidade_sel:
        df_filtrado = df_filtrado[df_filtrado['unidade'] == unidade_sel]
    if andar_sel:
        df_filtrado = df_filtrado[df_filtrado['andar'] == andar_sel]
    if medico_sel and medico_sel != "(Todos)":
        df_filtrado = df_filtrado[df_filtrado['medico'] == medico_sel]
    df_filtrado = df_filtrado.sort_values(by=["leito"]).reset_index(drop=True)
    # Exibe cada leito como bloco (linha identificação + ficha clínica abaixo)
    for _, leito in df_filtrado.iterrows():
        # Linha de identificação
        cols = st.columns([1, 3, 2, 2])
        cols[0].markdown(f"**Leito:** {leito['leito']}")
        cols[1].markdown(f"**Nome:** {leito.get('nome','')}")
        cols[2].markdown(f"**Médico:** {leito.get('medico','')}")
        cols[3].markdown(f"**Equipe:** {leito.get('equipe','')}")
        # Ficha clínica destacada
