...
# Botão manual para exportar dados de leitos para Google Sheets
if st.sidebar.button('Exportar dados de leitos para Google Sheets'):
    try:
        df_leitos = pd.read_csv(os.path.join('data', 'leitos.csv'))
        salvar_df_no_google_sheets(
            df_leitos,
            'hospital_app_dados',  # Nome da sua planilha
            'leitos',              # Nome da aba
            None                   # cred_path=None para compatibilidade
        )
...
# Botão manual para importar dados de leitos do Google Sheets
if st.sidebar.button('Importar dados do Google Sheets para Leitos'):
    try:
...
# Botão manual para exportar dados de médicos para Google Sheets
if st.sidebar.button('Exportar dados de médicos para Google Sheets'):
    try:
        ...
        st.error(f'Falha ao exportar dados de médicos para Google Sheets: {e}')

# Botão manual para importar dados de médicos do Google Sheets
if st.sidebar.button('Importar dados do Google Sheets para Médicos'):
    try:
        from google_sheets_utils import importar_df_do_google_sheets
...
if aba_selecionada == "🧮 IRAH":
    ...
    if os.path.exists(CAMINHO_LEITOS):
        ...
        # Botão para atualizar ficha clínica
        ...
        st.markdown("---")
        st.subheader("Histórico local do IRAH")
        historico_path = os.path.join('data', 'historico_irah.csv')
        if os.path.exists(historico_path):
            df_hist_irah = pd.read_csv(historico_path, dtype=str).fillna("")
            st.dataframe(df_hist_irah, use_container_width=True, hide_index=True)
        else:
            st.info("Nenhum histórico IRAH registrado.")
    else:
        st.info("Nenhum dado de leitos disponível.")
    # --- Integração do histórico do dashboard (Google Sheets) ---
    st.markdown("---")
    st.subheader("Histórico global do IRAH (Dashboard)")
    try:
        import json
        import gspread
        from oauth2client.service_account import ServiceAccountCredentials
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        # Corrigir: garantir que as credenciais sejam dict, não AttrDict
        import collections.abc
        def attrdict_to_dict(obj):
            if isinstance(obj, collections.abc.Mapping):
                return {k: attrdict_to_dict(v) for k, v in obj.items()}
            elif isinstance(obj, list):
                return [attrdict_to_dict(i) for i in obj]
            else:
                return obj
        credentials_dict = attrdict_to_dict(st.secrets["gcp_service_account"])
        ...
        if not dados.empty and "irah" in dados.columns:
            st.dataframe(dados, use_container_width=True, hide_index=True)
        else:
            st.info("O dashboard global ainda não possui registros válidos.")
    except Exception as e:
        st.error(f"Erro ao acessar o dashboard global: {e}")
