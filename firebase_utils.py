...
def inicializar_firebase():
    ...
    if not firebase_initialized:
        ...
        print("DEBUG: Tipo de st.secrets['FIREBASE']:", type(st.secrets.get('FIREBASE')))
        print("DEBUG: Valor parcial de st.secrets['FIREBASE']:", str(st.secrets.get('FIREBASE'))[:100], '...')
        print("DEBUG: Tipo de st.secrets['DATABASE_URL_CONFIG']:", type(st.secrets.get('DATABASE_URL_CONFIG')))
        print("DEBUG: Valor de st.secrets['DATABASE_URL_CONFIG']:", st.secrets.get('DATABASE_URL_CONFIG'))
        try:
            import json
            firebase_secret = st.secrets["FIREBASE"]
            if isinstance(firebase_secret, str):
                firebase_secret = json.loads(firebase_secret)
            print("DEBUG: Campos presentes em firebase_secret:", list(firebase_secret.keys()))
            print("DEBUG: Início de private_key:", firebase_secret.get('private_key', '')[:40])
            print("DEBUG: Tamanho de private_key:", len(firebase_secret.get('private_key', '')))
